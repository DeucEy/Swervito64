...
LRESULT CSupportEnterCode::OnOkCmd(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL & /*bHandled*/)
{
    ...
    {
        MessageBox(wGS(MSG_SUPPORT_ENTER_SUPPORT_CODE).c_str(), wGS(MSG_SUPPORT_Swervito64).c_str(), MB_OK);
        return false;
    }
    ...
    if (_wcsicmp(code, L"thank you from Swervito64") == 0)
    {
    ...
    if (ValidCode)
    {
        MessageBox(wGS(MSG_SUPPORT_COMPLETE).c_str(), wGS(MSG_SUPPORT_Swervito64).c_str(), MB_OK);
        EndDialog(wID);
    }
    else
    {
        MessageBox(wGS(MSG_SUPPORT_INCORRECT_CODE).c_str(), wGS(MSG_SUPPORT_Swervito64).c_str(), MB_OK);
        GetDlgItem(IDOK).EnableWindow(TRUE);
        GetDlgItem(IDCANCEL).EnableWindow(TRUE);
    }
    return TRUE;
...
LRESULT CRequestCode::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL & /*bHandled*/)
{
    if (m_ShowOldCodeMsg)
    {
        SetDlgItemText(IDC_DESCRIPTION, L"We have changed the code to be unique to a machine, please enter the email you used to support Swervito64 with.");
    }
    return TRUE;
}
