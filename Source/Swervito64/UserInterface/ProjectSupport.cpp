...
{
    if (Email == nullptr || strlen(Email) == 0 || _stricmp(Email, "thank you from Swervito64") == 0)
    {
        return false;
    }
    ...
    DWORD StatusCode;
    if (!PerformRequest(L"/index.php?option=com_Swervito64", PostData, StatusCode, Headers))
    {
        return false;
    }
    if (StatusCode == 200)
...
bool CProjectSupport::ValidateCode(const char * Code)
{
    ...
    std::vector<std::string> Headers;
    DWORD StatusCode;
    if (PerformRequest(L"/index.php?option=com_Swervito64", PostData, StatusCode, Headers) && StatusCode == 200)
    {
...
bool CProjectSupport::PerformRequest(const wchar_t * Url, const std::string & PostData, DWORD & StatusCode, std::vector<std::string> & Headers)
{
    ...
    HINTERNET hSession = InternetOpen(L"Swervito64", INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
    if (hSession == nullptr)
    {
        return false;
    }
...
void CProjectSupport::SaveSupportInfo(void)
{
    ...
    HKEY hKeyResults = 0;
    DWORD Disposition = 0;
    long lResult = RegCreateKeyEx(HKEY_CURRENT_USER, L"SOFTWARE\\Swervito64", 0, L"", REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &hKeyResults, &Disposition);
...
void CProjectSupport::LoadSupportInfo(void)
{
    ...
    HKEY hKeyResults = 0;
    long lResult = RegOpenKeyEx(HKEY_CURRENT_USER, L"SOFTWARE\\Swervito64", 0, KEY_READ, &hKeyResults);
    if (lResult == ERROR_SUCCESS)
    {
        DWORD DataSize = 0;
