...
#include <Swervito64/UserInterface/Debugger/DebuggerUI.h>
...
{
    std::wstring VersionDisplay = stdstr_f("Swervito64 %s", VER_FILE_VERSION_STR).ToUTF16();
...
{
    stdstr_f VersionDisplay("Swervito64 %s", VER_FILE_VERSION_STR);
...
WPARAM CMainGui::ProcessAllMessages(void)
{
    ...
    while (GetMessage(&msg, nullptr, 0, 0))
    {
        ...
        if (RA_IsOverlayFullyVisible())
        {
            ...
            // only do so when the overlay is fully visibile
            do
            {
                RA_ProcessInputs();
