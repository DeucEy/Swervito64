...
{
#ifdef _DEBUG
    TraceSetMaxModule(MaxTraceModuleSwervito64, TraceInfo);
#else
    TraceSetMaxModule(MaxTraceModuleSwervito64, TraceError);
#endif
    SetTraceModuleNames();
}
...
{
    CPath LogFilePath(g_Settings->LoadStringVal(Directory_Log).c_str(), "Swervito64.log");
...
{
    static stdstr_f ApplicationName("Swervito64 %s", VER_FILE_VERSION_STR);
