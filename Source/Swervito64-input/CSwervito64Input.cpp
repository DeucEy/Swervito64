#include "CSwervito64Input.h"
#include "InputSettings.h"
#include "InputConfigUI.h"

CSwervito64Input * g_InputPlugin = nullptr;

CSwervito64Input::CSwervito64Input(HINSTANCE hinst) :
    m_hinst(hinst),
    m_Scanning(false),
    m_DisplayCtrlId(0),
...
CSwervito64Input::~CSwervito64Input()
{
}

void CSwervito64Input::DevicesChanged(void)
{
    CGuard guard(m_CS);
...
void CSwervito64Input::DeviceAdded(void)
{
    ConfigUIDeviceAdded();
}

void CSwervito64Input::InitiateControllers(CONTROL_INFO * ControlInfo)
{
    CGuard guard(m_CS);
...
void CSwervito64Input::GetKeys(int32_t Control, BUTTONS * Keys)
{
...
void CSwervito64Input::StartScanDevices(int32_t DisplayCtrlId)
{
    m_Scanning = true;
...
void CSwervito64Input::EndScanDevices(void)
{
    m_Scanning = false;
    m_DisplayCtrlId = 0;
}

CDirectInput::ScanResult CSwervito64Input::ScanDevices(BUTTON & Button)
{
...
std::wstring CSwervito64Input::ButtonAssignment(BUTTON & Button)
{
    if (m_DirectInput.get() != nullptr)
    {
...
std::wstring CSwervito64Input::ControllerDevices(const N64CONTROLLER & Controller)
{
    if (m_DirectInput.get() != nullptr)
    {
...
bool CSwervito64Input::SaveController(uint32_t ControlIndex)
{
    CGuard guard(m_CS);
...
bool CSwervito64Input::ResetController(uint32_t ControlIndex, CONTROL & ControlInfo, N64CONTROLLER & Controller)
{
...
void CSwervito64Input::CheckShortcuts()
{
...
bool CSwervito64Input::SaveShortcuts()
{
    CGuard guard(m_CS);
...
bool CSwervito64Input::ResetShortcuts(SHORTCUTS& Shortcuts)
{
...
void CSwervito64Input::LockMouse()
{
...
void CSwervito64Input::UnlockMouse()
{
...
void CSwervito64Input::LockMouseSwitch()
{
    if (m_MouseLock == true)
    {
        UnlockMouse();
...
bool CSwervito64Input::IsMouseUsed()
{
    for (uint32_t i = 0, n = sizeof(m_Controllers) / sizeof(m_Controllers[0]); i < n; i++)
    {
...
void CSwervito64Input::LockCursor()
{
    RECT rect;
