...
#ifdef _DEBUG
    sprintf(PluginInfo->Name, "Swervito64 input plugin (Debug): %s", VER_FILE_VERSION_STR);
#else
    sprintf(PluginInfo->Name, "Swervito64 input plugin: %s", VER_FILE_VERSION_STR);
#endif
...
extern "C" int WINAPI DllMain(HINSTANCE hinst, DWORD fdwReason, LPVOID /*lpReserved*/)
{
    ...
    {
        g_InputPlugin = new CSwervito64Input(hinst);
    }
